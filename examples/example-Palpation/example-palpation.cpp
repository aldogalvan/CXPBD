...
void updateHaptics(void)
{
    ...
    // pointer variables
    float ppos[3]; float ppos_last[3]; float pproxy[3];

    // get initial position
    hapticDevice->getPosition(pos);
    ppos[0] = (float)pos(0); ppos[1] = (float)pos(1); ppos[2] = (float)pos(2);
    ppos_last[0] = (float)pos(0); ppos_last[1] = (float)pos(1); ppos_last[2] = (float)pos(2);
    pproxy[0] = (float)pos(0); pproxy[1] = (float)pos(1); pproxy[2] = (float)pos(2);
    ...
    while(simulationRunning) {
        ...
        *ppos_last = *ppos;
        ...
        hapticDevice->getPosition(pos);
        pos = wss*pos;
        ppos[0] = 1e3*pos(0); ppos[1] = 1e3*pos(1) ; ppos[2] = 1e3*pos(2);
        ...
        float* force;
        sim->updateDynamics(ppos,ppos_last,pproxy,force, 0,0.01);
